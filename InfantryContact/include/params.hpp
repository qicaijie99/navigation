...
struct Params
{
    ...
    struct Rx
    {
        ...
        bool gimbal_status_verbose = true;
        bool response_verbose      = true;
    };

    struct Tx
    {
        bool query_send_verbose      = false;
        bool send_cmd_verbose        = true;
        bool query_scheduler_enabled = false;
    };
    ...
    void declare_on(rclcpp::Node& node) {
        ...
        node.declare_parameter<bool>("rx.gimbal_status_verbose", true);
        node.declare_parameter<bool>("rx.response_verbose", true);

        // Tx set
        node.declare_parameter<bool>("tx.query_send_verbose", false);
        node.declare_parameter<bool>("tx.send_cmd_verbose", true);
        node.declare_parameter<bool>("tx.query_scheduler_enabled", false);
    ...
    static Params load(rclcpp::Node& node) {
        ...
        p.rx.gimbal_status_verbose = node.get_parameter("rx.gimbal_status_verbose").as_bool();
        p.rx.response_verbose      = node.get_parameter("rx.response_verbose").as_bool();

        // Tx
        p.tx.query_send_verbose      = node.get_parameter("tx.query_send_verbose").as_bool();
        p.tx.send_cmd_verbose        = node.get_parameter("tx.send_cmd_verbose").as_bool();
        p.tx.query_scheduler_enabled = node.get_parameter("tx.query_scheduler_enabled").as_bool();
        ...
        if (std::filesystem::exists(p.port.port_name) == false) {
            ...
            for (const auto& entry : std::filesystem::directory_iterator("/dev")) {
                ...
                if (path_str.find("ttyACM") != std::string::npos) {
                    ...
                    RCLCPP_WARN(node.get_logger(), "Using first available ttyACM port: %s", p.port.port_name.c_str());
                    break;
                }
            }
        }
        ...
        if (result == 0) {
            RCLCPP_INFO(node.get_logger(), "Successfully set permissions for %s", p.port.port_name.c_str());
        }
        else {
            RCLCPP_WARN(node.get_logger(), "Failed to set permissions for %s, you may need sudo", p.port.port_name.c_str());
        }
        ...
        std::cout << "\n========== RX Log ==========" << std::endl;
        std::cout << "GimbalStatus verbose: " << (p.rx.gimbal_status_verbose ? "Enabled" : "Disabled") << std::endl;
        std::cout << "Response verbose: " << (p.rx.response_verbose ? "Enabled" : "Disabled") << std::endl;
        std::cout << "============================" << std::endl;

        std::cout << "\n========== TX Log ==========" << std::endl;
        std::cout << "QuerySend verbose: " << (p.tx.query_send_verbose ? "Enabled" : "Disabled") << std::endl;
        std::cout << "SendCmd verbose: " << (p.tx.send_cmd_verbose ? "Enabled" : "Disabled") << std::endl;
        std::cout << "QueryScheduler: " << (p.tx.query_scheduler_enabled ? "Enabled" : "Disabled") << std::endl;
        std::cout << "============================" << std::endl;
