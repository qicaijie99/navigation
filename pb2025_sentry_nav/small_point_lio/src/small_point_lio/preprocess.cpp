...
namespace small_point_lio {
    ...
    void Preprocess::on_point_cloud_callback(const std::vector<common::Point> &pointcloud) {
        ...
        for (size_t i = 0; i < pointcloud.size(); i++) {
            ...
            const size_t lidar_id = static_cast<size_t>(point.lidar_id);
            if (lidar_id < parameters->lidar_time_offsets.size()) {
                point.timestamp += parameters->lidar_time_offsets[lidar_id];
            }
