...
namespace small_point_lio {
    ...
    void SmallPointLio::handle_once() {
        ...
        if (!is_init) {
            if ((!preprocess.point_deque.empty() || !preprocess.imu_deque.empty()) &&
                ...
                for (const auto &point: preprocess.point_deque) {
                    estimator.ivox->add_point(
                            estimator.point_lidar_to_imu(point.position, point.lidar_id).cast<float>());
                }
        ...
        while (!preprocess.imu_deque.empty() && !preprocess.dense_point_deque.empty() && !preprocess.point_deque.empty()) {
            ...
            if (dense_point_lidar_frame.timestamp < point_lidar_frame.timestamp && dense_point_lidar_frame.timestamp < imu_msg.timestamp) {
                ...
                Eigen::Matrix<state::value_type, 3, 1> dense_point_imu_frame =
                        estimator.point_lidar_to_imu(dense_point_lidar_frame.position, dense_point_lidar_frame.lidar_id);
                pointcloud_body_frame.emplace_back(dense_point_imu_frame.cast<float>());
                ...
                estimator.point_lidar_id = point_lidar_frame.lidar_id;
        ...
        if (is_publish_odometry) {
            ...
            if (!pointcloud_body_frame.empty()) {
                if (pointcloud_body_callback) {
                    pointcloud_body_callback(pointcloud_body_frame);
                }
                pointcloud_body_frame.clear();
            }
        }
    ...
    void SmallPointLio::set_pointcloud_body_callback(const std::function<void(const std::vector<Eigen::Vector3f> &pointcloud)> &pointcloud_body_callback) {
        this->pointcloud_body_callback = pointcloud_body_callback;
    }
