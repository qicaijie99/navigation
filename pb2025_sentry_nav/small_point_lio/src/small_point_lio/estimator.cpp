...
namespace small_point_lio {
    ...
    [[nodiscard]] Eigen::Matrix<state::value_type, 3, 1> Estimator::point_lidar_to_imu(
            const Eigen::Vector3f &point_lidar_frame,
            uint8_t lidar_id) const {
        if (parameters->extrinsic_est_en) {
            return kf.x.offset_R_L_I * point_lidar_frame.cast<state::value_type>() + kf.x.offset_T_L_I;
        }
        if (parameters->multi_lidar_enabled && !parameters->lidar_R_wrt_IMU.empty()) {
            int lidar_index = static_cast<int>(lidar_id);
            if (lidar_index < 0 || lidar_index >= static_cast<int>(parameters->lidar_R_wrt_IMU.size())) {
                lidar_index = 0;
            }
            return parameters->lidar_R_wrt_IMU[static_cast<size_t>(lidar_index)].cast<state::value_type>() *
                           point_lidar_frame.cast<state::value_type>() +
                   parameters->lidar_T_wrt_IMU[static_cast<size_t>(lidar_index)].cast<state::value_type>();
        }
        return Lidar_R_wrt_IMU * point_lidar_frame.cast<state::value_type>() + Lidar_T_wrt_IMU;
    }

    void Estimator::h_point(const state &s, point_measurement_result &measurement_result) {
        measurement_result.valid = false;
        // get closest point
        const Eigen::Matrix<state::value_type, 3, 1> point_imu_frame =
                point_lidar_to_imu(point_lidar_frame, point_lidar_id);
