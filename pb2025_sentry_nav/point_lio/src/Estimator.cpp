...
std::vector<V3D> Lidar_Ts_wrt_IMU;
std::vector<M3D> Lidar_Rs_wrt_IMU;
double G_m_s2 = 9.81;
...
int pointLidarId(PointType const * const pi)
{
  if (!multi_lidar_enabled || pi == nullptr) {
    return 0;
  }

  int lidar_id = static_cast<int>(std::lround(pi->normal_x));
  if (lidar_id < 0 || lidar_id >= static_cast<int>(Lidar_Rs_wrt_IMU.size())) {
    lidar_id = 0;
  }
  return lidar_id;
}

V3D pointLidarToIMU(PointType const * const pi)
{
  V3D p_body_lidar(pi->x, pi->y, pi->z);
  if (multi_lidar_enabled && !Lidar_Rs_wrt_IMU.empty()) {
    const int lidar_id = pointLidarId(pi);
    return Lidar_Rs_wrt_IMU[lidar_id] * p_body_lidar +
           Lidar_Ts_wrt_IMU[lidar_id];
  }

  if (extrinsic_est_en) {
    if (!use_imu_as_input) {
      return kf_output.x_.offset_R_L_I * p_body_lidar + kf_output.x_.offset_T_L_I;
    }
    return kf_input.x_.offset_R_L_I * p_body_lidar + kf_input.x_.offset_T_L_I;
  }

  return Lidar_R_wrt_IMU * p_body_lidar + Lidar_T_wrt_IMU;
}

void pointBodyToWorld(PointType const * const pi, PointType * const po)
{
  const V3D p_imu = pointLidarToIMU(pi);

  V3D p_global;
  if (!use_imu_as_input) {
    p_global = kf_output.x_.rot * p_imu + kf_output.x_.pos;
  } else {
    p_global = kf_input.x_.rot * p_imu + kf_input.x_.pos;
  }
