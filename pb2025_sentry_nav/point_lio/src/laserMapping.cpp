...
{
  V3D p_body_imu = pointLidarToIMU(pi);
  po->x = p_body_imu(0);
  po->y = p_body_imu(1);
  po->z = p_body_imu(2);
...
int main(int argc, char ** argv)
{
  ...
  Lidar_Ts_wrt_IMU.clear();
  Lidar_Rs_wrt_IMU.clear();
  auto append_lidar_extrinsic =
    [&](const std::vector<double> & trans, const std::vector<double> & rot, const int lidar_id) {
      V3D lidar_t = Lidar_T_wrt_IMU;
      M3D lidar_r = Lidar_R_wrt_IMU;
      if (trans.size() == 3) {
        lidar_t << VEC_FROM_ARRAY(trans);
      } else {
        RCLCPP_WARN(
          nh->get_logger(),
          "multi_lidar.lidar%d.extrinsic_T must have 3 values. Using mapping.extrinsic_T",
          lidar_id);
      }
      if (rot.size() == 9) {
        lidar_r << MAT_FROM_ARRAY(rot);
      } else {
        RCLCPP_WARN(
          nh->get_logger(),
          "multi_lidar.lidar%d.extrinsic_R must have 9 values. Using mapping.extrinsic_R",
          lidar_id);
      }
      Lidar_Ts_wrt_IMU.emplace_back(lidar_t);
      Lidar_Rs_wrt_IMU.emplace_back(lidar_r);
    };
  append_lidar_extrinsic(extrinT_lidar0, extrinR_lidar0, 0);
  if (multi_lidar_count > 1) {
    append_lidar_extrinsic(extrinT_lidar1, extrinR_lidar1, 1);
  }
  ...
  const bool use_pc2_for_avia =
    (lid_topic.find("fused_cloud") != std::string::npos) ||
    (lid_topic.find("multi_lidar_points") != std::string::npos);

  if (p_pre->lidar_type == AVIA && !use_pc2_for_avia) {
  ...
  while (rclcpp::ok()) {
    ...
    if (sync_packages(Measures)) {
      ...
      for (size_t i = 0; i < feats_down_body->size(); i++) {
        ...
        V3D point_this =
          (!extrinsic_est_en || multi_lidar_enabled)
            ? pointLidarToIMU(&feats_down_body->points[i])
            : point_lidar;
