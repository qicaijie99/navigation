...
extern std::vector<double> lidar_time_offsets;
...
std::optional<PointField> findField(const PointCloud2 & cloud, const std::string & name)
{
...
template<typename T>
T readUnchecked(const PointCloud2 & cloud, const std::size_t base, const PointField & field)
{
  T value{};
  std::memcpy(&value, &cloud.data[base + field.offset], sizeof(T));
  return value;
}

double readNumeric(
  const PointCloud2 & cloud, const std::size_t base, const std::optional<PointField> & field,
  const double fallback)
{
  if (!field || base + field->offset >= cloud.data.size()) {
    return fallback;
  }

  switch (field->datatype) {
    case PointField::INT8:
      return static_cast<double>(readUnchecked<int8_t>(cloud, base, *field));
    case PointField::UINT8:
      return static_cast<double>(readUnchecked<uint8_t>(cloud, base, *field));
    case PointField::INT16:
      return static_cast<double>(readUnchecked<int16_t>(cloud, base, *field));
    case PointField::UINT16:
      return static_cast<double>(readUnchecked<uint16_t>(cloud, base, *field));
    case PointField::INT32:
      return static_cast<double>(readUnchecked<int32_t>(cloud, base, *field));
    case PointField::UINT32:
      return static_cast<double>(readUnchecked<uint32_t>(cloud, base, *field));
    case PointField::FLOAT32:
      return static_cast<double>(readUnchecked<float>(cloud, base, *field));
    case PointField::FLOAT64:
      return readUnchecked<double>(cloud, base, *field);
    default:
      return fallback;
  }
}

double lidarTimeOffsetNs(const int lidar_id)
{
  if (lidar_id < 0 || lidar_id >= static_cast<int>(lidar_time_offsets.size())) {
    return 0.0;
  }
  return lidar_time_offsets[static_cast<std::size_t>(lidar_id)] * 1.0e9;
}
}  // namespace
...
void Preprocess::avia_handler_pc2(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  for (int i = 0; i < plsize; ++i) {
    ...
    // livox_ros_driver2 的 PointCloud2 里 timestamp 字段对应逐点时间，按 ns 处理，转成 ms 存到 curvature
    pt.curvature = static_cast<float>(src.timestamp * 1e-6);

    const double dist = pt.x * pt.x + pt.y * pt.y + pt.z * pt.z;
    if (dist < blind * blind || dist > det_range * det_range) {
      continue;
    }
  ...
  std::sort(pl_surf.points.begin(), pl_surf.points.end(), time_list_cut_frame);
}

void Preprocess::livox_pcl2_handler(
  const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  const auto lidar_id_field = findField(*msg, "lidar_id");
  ...
  double min_ts_ns = std::numeric_limits<double>::max();
  const double fallback_ts_ns = rclcpp::Time(msg->header.stamp).seconds() * 1.0e9;

  for (int i = 0; i < plsize; ++i) {
    const std::size_t base = static_cast<std::size_t>(i) * msg->point_step;
    if (base + msg->point_step > msg->data.size()) {
      break;
    }

    const float x = static_cast<float>(readNumeric(*msg, base, x_field, 0.0));
    const float y = static_cast<float>(readNumeric(*msg, base, y_field, 0.0));
    const float z = static_cast<float>(readNumeric(*msg, base, z_field, 0.0));
    if (std::isnan(x) || std::isnan(y) || std::isnan(z)) {
      continue;
    }

    const int lidar_id = static_cast<int>(std::lround(
      readNumeric(*msg, base, lidar_id_field, 0.0)));
    const double timestamp_ns =
      readNumeric(*msg, base, timestamp_field, fallback_ts_ns) + lidarTimeOffsetNs(lidar_id);
    if (timestamp_ns < min_ts_ns) {
      min_ts_ns = timestamp_ns;
    }
  }

  if (!std::isfinite(min_ts_ns)) {
    return;
  }
  ...
  for (int i = 0; i < plsize; ++i) {
    ...
    PointType added_pt;
    const int lidar_id = std::clamp(
      static_cast<int>(std::lround(readNumeric(*msg, base, lidar_id_field, 0.0))), 0, 255);
    added_pt.normal_x = static_cast<float>(lidar_id);
    added_pt.normal_y = 0.0f;
    added_pt.normal_z = 0.0f;

    added_pt.x = static_cast<float>(readNumeric(*msg, base, x_field, 0.0));
    added_pt.y = static_cast<float>(readNumeric(*msg, base, y_field, 0.0));
    added_pt.z = static_cast<float>(readNumeric(*msg, base, z_field, 0.0));
    added_pt.intensity = static_cast<float>(readNumeric(*msg, base, intensity_field, 0.0));

    const double timestamp_ns =
      readNumeric(*msg, base, timestamp_field, fallback_ts_ns) + lidarTimeOffsetNs(lidar_id);
    // Store per-point time relative to this measurement batch, unit: ms.
    added_pt.curvature = static_cast<float>((timestamp_ns - min_ts_ns) / 1000000.0);

    double dist =
      added_pt.x * added_pt.x +
  ...
  if (!pl_surf.empty()) {
    //std::cout
      //<< "[DEBUG] livox_pcl2_handler in=" << plsize
      //<< " out=" << pl_surf.size()
      //<< " span_ms=" << pl_surf.back().curvature
      //<< std::endl;
  } else {
    //std::cout << "[DEBUG] livox_pcl2_handler empty cloud after filter" << std::endl;
  }
}
