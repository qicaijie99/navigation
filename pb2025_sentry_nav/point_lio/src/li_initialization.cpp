...
void standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::SharedPtr & msg)
{
  ...
  if ((lidar_type == VELO16 || lidar_type == OUST64 || lidar_type == HESAIxt32) && cut_frame_init) {
    ...
    static int pcl2_dbg_cnt = 0;
    if ((pcl2_dbg_cnt++ % 20) == 0) {
      if (!ptr->points.empty()) {
        RCLCPP_INFO(
          rclcpp::get_logger("li_initialization"),
          "[DBG] standard_pcl_cbk: msg_width=%u frame=%s ptr_size=%zu first_xyz=(%.3f, %.3f, %.3f) first_curv=%.6f",
          msg->width,
          ...
          ptr->points[0].y,
          ptr->points[0].z,
          ptr->points[0].curvature
        );
      } else {
        RCLCPP_WARN(
          rclcpp::get_logger("li_initialization"),
          "[DBG] standard_pcl_cbk: msg_width=%u frame=%s ptr_size=0",
          msg->width,
          msg->header.frame_id.c_str()
        );
      }
