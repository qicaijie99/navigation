...
int multi_lidar_count = 1;
std::vector<double> extrinT_lidar0;
std::vector<double> extrinR_lidar0;
std::vector<double> extrinT_lidar1;
std::vector<double> extrinR_lidar1;
std::vector<double> lidar_time_offsets;
...
void readParameters(std::shared_ptr<rclcpp::Node> & nh)
{
  ...
  try {
    ...
    nh->declare_parameter<bool>("multi_lidar.enable", false);
    nh->get_parameter("multi_lidar.enable", multi_lidar_enabled);

    nh->declare_parameter<int>("multi_lidar.lidar_count", 1);
    nh->get_parameter("multi_lidar.lidar_count", multi_lidar_count);

    nh->declare_parameter<std::vector<double>>(
      "multi_lidar.lidar0.extrinsic_T", std::vector<double>());
    nh->get_parameter("multi_lidar.lidar0.extrinsic_T", extrinT_lidar0);

    nh->declare_parameter<std::vector<double>>(
      "multi_lidar.lidar0.extrinsic_R", std::vector<double>());
    nh->get_parameter("multi_lidar.lidar0.extrinsic_R", extrinR_lidar0);

    nh->declare_parameter<std::vector<double>>(
      "multi_lidar.lidar1.extrinsic_T", std::vector<double>());
    nh->get_parameter("multi_lidar.lidar1.extrinsic_T", extrinT_lidar1);

    nh->declare_parameter<std::vector<double>>(
      "multi_lidar.lidar1.extrinsic_R", std::vector<double>());
    nh->get_parameter("multi_lidar.lidar1.extrinsic_R", extrinR_lidar1);

    nh->declare_parameter<std::vector<double>>(
      "multi_lidar.time_offsets", std::vector<double>());
    nh->get_parameter("multi_lidar.time_offsets", lidar_time_offsets);
  ...
  if (extrinT_lidar0.empty()) {
    extrinT_lidar0 = extrinT;
  }
  if (extrinR_lidar0.empty()) {
    extrinR_lidar0 = extrinR;
  }
  if (extrinT_lidar1.empty()) {
    extrinT_lidar1 = extrinT_lidar0;
  }
  if (extrinR_lidar1.empty()) {
    extrinR_lidar1 = extrinR_lidar0;
  }
  if (multi_lidar_count < 1) {
    RCLCPP_WARN(nh->get_logger(), "multi_lidar.lidar_count < 1, forcing it to 1");
    multi_lidar_count = 1;
  }
  if (multi_lidar_count > 2) {
    RCLCPP_WARN(
      nh->get_logger(),
      "This Point-LIO adapter currently supports lidar_id 0 and 1. Forcing lidar_count to 2");
    multi_lidar_count = 2;
  }
  if (lidar_time_offsets.size() < static_cast<std::size_t>(multi_lidar_count)) {
    lidar_time_offsets.resize(static_cast<std::size_t>(multi_lidar_count), 0.0);
  }
  if (multi_lidar_enabled && extrinsic_est_en) {
    RCLCPP_WARN(
      nh->get_logger(),
      "multi_lidar.enable=true requires fixed per-lidar extrinsics. Forcing "
