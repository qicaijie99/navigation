...
//builtin_interfaces::msg::Time laserCloudStamp;
...
{
  //laserCloudStamp = laserCloud2->header.stamp;
...
int main(int argc, char ** argv)
{
  ...
  nh->declare_parameter<double>("publishStampLag", publishStampLag);
  ...
  nh->get_parameter("publishStampLag", publishStampLag);
  publishStampLag = std::max(0.0, publishStampLag);
  ...
  while (status) {
    ...
    if (newlaserCloud) {
      ...
      //terrainCloud2.header.stamp =
        //nh->now() - rclcpp::Duration::from_seconds(publishStampLag);
