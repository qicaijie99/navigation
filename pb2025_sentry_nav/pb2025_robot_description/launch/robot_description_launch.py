...
def launch_setup(context: LaunchContext) -> list:
    ...
    # joint_state_publisher in this workspace expects a robot description file
    # or a robot_description topic. Provide a temporary URDF file directly so it
    # can publish default zero joint states even when no external source exists.
    with tempfile.NamedTemporaryFile(
        mode="w", suffix=".urdf", prefix="pb2025_robot_description_", delete=False
    ) as urdf_file:
        urdf_file.write(robot_urdf_xml)
        joint_state_description_file = urdf_file.name
